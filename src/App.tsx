...
function EventPage({ id }: { id: string }) {
  ...
  const submit = async (exclude = excluded) => {
    try {
      const updated = await saveResponse(id, selectedPerson, draft, false, exclude)
  ...
  const toggleBooking = async (slotId: string) => {
    try { setEvent(await bookEvent(id, slotId, selectedPerson)); setLoadError('') }
